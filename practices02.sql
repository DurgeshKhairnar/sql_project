CREATE DATABASE PracticeDB3;
USE PracticeDB3;

CREATE TABLE Departments(
	dept_id INT primary key,
    dept_name varchar(50)
);

INSERT INTO departments values
(101,"IT"),
(102,"HR"),
(103,"Finance"),
(104,"Marketing"),
(105,"Sales"),
(106,"Operations");

CREATE TABLE Employees(
	emp_id INT PRIMARY KEY,
    emp_name varchar(50),
    dept_id int,
    salary int,
    city varchar(50),
    join_date DATE,
    foreign key (dept_id)
    references departments(dept_id)
);

INSERT INTO employees values
(1,"Durgesh",101,"95000","Pune","2022-01-15"),
(2,'Sai',102,50000,'Chennai','2021-05-10'),
(3,'Vishal',101,75000,'Delhi','2020-08-20'),
(4,'Rahul',103,55000,'Mumbai','2022-03-11'),
(5,'Anu',102,45000,'Pune','2023-02-01'),
(6,'Kiran',104,60000,'Bangalore','2019-11-15'),
(7,'Neha',103,58000,'Delhi','2021-04-22'),
(8,'Priya',104,62000,'Mumbai','2020-09-30'),
(9,'Amit',105,52000,'Chennai','2022-12-05'),
(10,'Pooja',105,70000,'Pune','2021-08-18');


CREATE TABLE Orders (
    order_id INT PRIMARY KEY,
    emp_id INT,
    product_name VARCHAR(50),
    category VARCHAR(50),
    amount INT,
    order_date DATE,
    FOREIGN KEY (emp_id)
    REFERENCES Employees(emp_id)
);

INSERT INTO Orders VALUES
(101,1,'Laptop','Electronics',50000,'2024-01-05'),
(102,2,'Phone','Electronics',25000,'2024-01-07'),
(103,1,'Mouse','Accessories',1000,'2024-01-10'),
(104,3,'Tablet','Electronics',30000,'2024-01-15'),
(105,4,'Watch','Wearables',5000,'2024-01-20'),
(106,2,'Keyboard','Accessories',2000,'2024-01-22'),
(107,6,'Speaker','Accessories',4000,'2024-02-01'),
(108,8,'Laptop','Electronics',50000,'2024-02-05'),
(109,1,'Phone','Electronics',25000,'2024-02-10'),
(110,6,'Mouse','Accessories',1000,'2024-02-15'),
(111,3,'Laptop','Electronics',50000,'2024-03-01'),
(112,7,'Watch','Wearables',10000,'2024-03-08'),
(113,8,'Phone','Electronics',25000,'2024-03-15'),
(114,9,'Speaker','Accessories',4000,'2024-03-20'),
(115,1,'Tablet','Electronics',30000,'2024-03-28');

SELECT e.emp_name , d.dept_name
FROM employees e 
join departments d
on e.dept_id = d.dept_id;

SELECT emp_name , salary
FROM employees;

SELECT e.emp_name , o.amount
FROM employees e 
join orders o 
on e.emp_id = o.emp_id;

SELECT e.city , o.product_name
FROM employees e 
join orders o 
on e.emp_id = o.emp_id;

SELECT e.emp_name , sum(o.amount)
FROM employees e 
join orders o 
on e.emp_id = o.emp_id
group by e.emp_name;

SELECT e.emp_name , sum(o.amount) as tam
FROM employees e 
join orders o 
on e.emp_id = o.emp_id
group by e.emp_name
order by tam desc
limit 1;

SELECT e.emp_name , count(o.order_id) as co
FROM employees e 
join orders o 
on e.emp_id = o.emp_id
group by e.emp_name
order by co desc
limit 1;

SELECT e.city , sum(o.amount) as tam
FROM employees e 
join orders o 
on e.emp_id = o.emp_id
group by e.city;

SELECT d.dept_name , sum(o.amount) as tam
FROM departments d 
join employees e 
on d.dept_id = e.dept_id
join orders o 
on o.emp_id = e.emp_id
group by d.dept_name;

SELECT e.emp_name , o.order_id
from employees e 
left join orders o 
on e.emp_id = o.emp_id;

SELECT e.emp_name , o.order_id
from employees e 
left join orders o 
on e.emp_id = o.emp_id
where o.order_id is null;

SELECT e.emp_name , count(o.order_id) 
from employees e 
left join orders o 
on e.emp_id = o.emp_id
group by e.emp_name;

SELECT d.dept_name , e.emp_id 
from departments d 
left join employees e
on d.dept_id = e.dept_id;

SELECT * 
FROM employees
where salary > (
	SELECT avg(salary)
    FROM employees
);

SELECT * 
FROM employees
where salary < (
	SELECT avg(salary)
    FROM employees
);

SELECT * 
FROM employees
where salary = (
	SELECT max(salary)
    FROM employees
);

SELECT MAX(salary)
FROM employees
where salary < (
	SELECT max(salary)
    FROM employees
);

SELECT * 
FROM employees
where salary > (
	SELECT salary
    FROM employees
    where emp_name = "Sai"
);

SELECT *
FROM orders
where amount > (
	SELECT avg(amount)
    FROM orders 
);

SELECT d.dept_name , avg(e.salary) as davg
from departments d
join employees e 
on d.dept_id = e.dept_id 
group by d.dept_name
having davg > (
		SELECT avg(amount)
        FROM orders
);

SELECT city , avg(salary) as davg
from employees
group by city
having davg > (
		SELECT avg(amount)
        FROM orders
);

SELECT e.emp_name , sum(o.amount) as am
FROM employees e 
join orders o 
on e.emp_id = o.emp_id 
group by e.emp_name
having am > (
			SELECT avg(empoyees_sales)
            FROM(
					SELECT e.emp_name , sum(o.amount) as empoyees_sales
					FROM employees e 
					left join orders o 
					on e.emp_id = o.emp_id 
					group by e.emp_name
            )t

);


SELECT e.emp_name , count(o.order_id) as am
FROM employees e 
join orders o 
on e.emp_id = o.emp_id 
group by e.emp_name
having am > (
			SELECT avg(order_count)
            FROM(
					SELECT e.emp_name , count(o.order_id) as order_count
					FROM employees e 
					left join orders o 
					on e.emp_id = o.emp_id 
					group by e.emp_name
            )t

);

SELECT d.dept_name , avg(e.salary) avg_sal
from employees e 
join departments d 
on d.dept_id = e.dept_id 
group by d.dept_name
order by avg_sal desc
limit 1;

-- windows function 

SELECT *,
	row_number() over(order by salary desc)
    FROM employees;


SELECT *
FROM
(
    SELECT *,
           ROW_NUMBER() OVER(ORDER BY salary DESC) AS r
    FROM employees
) t
WHERE r <= 3;

SELECT *
FROM
(
    SELECT *,
           ROW_NUMBER() OVER(ORDER BY salary DESC) AS r
    FROM employees
) t
WHERE r = 1;

SELECT * 
FROM (
		SELECT * ,
        rank() over(order by salary desc)
        FROM employees
)t;

SELECT * 
FROM (
		SELECT * ,
        rank() over(order by salary desc) as r
        FROM employees
)t
where r = 1;

SELECT * 
FROM (
		SELECT * ,
        dense_rank() over(order by salary desc) as r
        FROM employees
)t
where r = 2;

SELECT dept_id,
		avg(salary)
        over(partition by dept_id)
        FROM employees;

SELECT dept_id,
count(emp_id)
over(partition by dept_id)
FROM employees;

SELECT emp_id,
	 amount,
	sum(amount)
    over(order by emp_id)
    FROM orders;
    
    
SELECT emp_id,
	 amount,
	avg(amount)
    over(order by emp_id)
    FROM orders;

SELECT * 
FROM (
		SELECT 
		order_id,
        amount,
        avg(amount)
        over(partition by order_id) avg_am
        FROM orders
)t
where amount < avg_am;

SELECT order_id,
	   order_date,
       amount,
       sum(amount)
       over(order by order_date)
       FROM orders;
SELECT order_id,
	   order_date,
       count(order_id)
       over(order by order_date)
       FROM orders;

SELECT order_id,
lag(amount)
over(order by order_date)
FROM orders;

SELECT order_id,
lead(amount)
over(order by order_date)
FROM orders;

SELECT order_id,
       amount,
       amount -
       LAG(amount)
       OVER(ORDER BY order_date) AS diff
FROM Orders;

SELECT * FROM departments;
SELECT * FROM employees;
SELECT * FROM orders;

